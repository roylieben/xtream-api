...
export function maybeRunDueSyncs(): void {
  (async () => {
    try {
      ...
      if (s.sync_auto_live !== false && due(s.last_sync_live_at, s.sync_interval_live_minutes)) syncLive().catch(() => {});
      if (s.sync_auto_vod !== false && due(s.last_sync_vod_at, s.sync_interval_vod_minutes)) syncVod().catch(() => {});
      if (s.sync_auto_series !== false && due(s.last_sync_series_at, s.sync_interval_series_minutes)) syncSeries().catch(() => {});
    } catch {
      /* swallow */
    }
  })();
