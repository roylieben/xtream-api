...
async function handle(request: Request): Promise<Response> {
  ...
  // Fire background sync check (non-blocking)
  maybeRunDueSyncs();
  ...
  switch (action) {
    ...
    case "get_vod_info": {
      ...
      const { data } = await supabaseAdmin.from("vod_info").select("info,movie_data").eq("vod_id", id).maybeSingle();
    ...
    case "get_simple_data_table": {
      // Pass through to upstream for EPG
